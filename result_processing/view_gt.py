...
def draw_bounding_box_class(
    org, components, color=C.COLOR, line=2, show=False, write_path=None
):
    """
    ...
    for i in range(len(bboxes)):
        ...
        board = cv2.rectangle(
            ...
            (bbox[0], bbox[1]),
            (bbox[2], bbox[3]),
            color[C.CLASS_MAP[str(category)]],
            line,
        )
        board = cv2.putText(
            board,
            C.CLASS_MAP[str(category)],
            (bbox[0] + 5, bbox[1] + 20),
...
def load_ground_truth_json(gt_file, no_text=True):
    ...
    for annot in tqdm(annots):
        ...
        if img_name not in compos:
            compos[img_name] = {
                "bboxes": [cvt_bbox(annot["bbox"])],
                "categories": [annot["category_id"]],
                "size": size,
            }
        else:
...
gt = load_ground_truth_json(
    "E:\\Mulong\\Datasets\\rico\\instances_test.json", no_text=False
)
