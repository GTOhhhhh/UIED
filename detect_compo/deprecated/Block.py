...
class Block(Component):
    ...
    def block_is_uicompo(self, image_shape, max_compo_scale):
        ...
        if (
            self.bbox.height / row > max_compo_scale[0]
            or self.bbox.width / column > max_compo_scale[1]
        ):
            return False
        return True
    ...
    def block_is_top_or_bottom_bar(self, image_shape, top_bottom_height):
        ...
        if (
            column_min < 5
            and row_min < 5
            and width - column_max < 5
            and row_max < height * top_bottom_height[0]
        ):
            self.uicompo_ = True
            return True
        if (
            column_min < 5
            and row_min > height * top_bottom_height[1]
            and width - column_max < 5
            and height - row_max < 5
        ):
            self.uicompo_ = True
            return True
        return False
